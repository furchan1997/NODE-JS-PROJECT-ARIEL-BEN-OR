...
// יצירת כרטיס חדש
router.post("/", authMW, bizAuthMW, async (req, res, next) => {
  try {
...
router.get("/my-cards", authMW, async (req, res, next) => {
  try {
...
router.patch("/admin/:id", authMW, async (req, res, next) => {
  try {
...
router.put("/:id", authMW, async (req, res, next) => {
  try {
    ...
    if (req.body.bizNumber) {
      return res
        .status(errors.BAD_REQUEST.status)
        .send(errors.BAD_REQUEST.message);
    }
    ...
    next(error); // העברת השגיאה למידלוואר של טיפול בשגיאות
  }
});
